...
<base-class> ::= `SmartContract' | `StatefulSmartContract' | `InductiveSmartContract'
...
\begin{definition}[Inductive Contract]
An \emph{inductive contract} (\texttt{InductiveSmartContract}) extends
\texttt{StatefulSmartContract} with backward chain verification: each
transaction verifies its parent transaction's authenticity, lineage
consistency, and chain linking, proving the entire UTXO history back to
genesis by mathematical induction.
\end{definition}

The \texttt{parentClass} field on the AST's \texttt{ContractNode}
discriminates between these three models, determining which compilation
...
\texttt{StatefulSmartContract} or \texttt{InductiveSmartContract} determines the contract model.
...
\begin{enumerate}[label=\textbf{V\arabic*}., ref=V\arabic*]
  ...
  \item \label{v:readonly} All properties in \texttt{SmartContract} are
    \texttt{readonly}. If a \texttt{StatefulSmartContract} or
    \texttt{InductiveSmartContract} has no mutable properties, a warning
    is emitted (suggesting \texttt{SmartContract} instead), but
...
For contracts extending \texttt{StatefulSmartContract} (or
\texttt{InductiveSmartContract}, which adds parent tx verification),
the ANF lowering pass automatically injects additional bindings at the
entry and exit of every public method:
