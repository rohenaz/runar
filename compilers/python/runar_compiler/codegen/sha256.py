...
Direct port of ``packages/runar-compiler/src/passes/sha256-codegen.ts``.
"""
...
class _Emitter:
    """Emitter with depth tracking for SHA-256 codegen."""
    ...
    def assert_depth(self, expected: int, msg: str) -> None:
        if self.depth != expected:
            raise RuntimeError(
                f"SHA256 codegen: {msg}. Expected depth {expected}, got {self.depth}"
            )
    ...
    def big_sigma0(self) -> None:
        """S0(a) = ROTR(2)^ROTR(13)^ROTR(22). [a(LE)] -> [S0(LE)]. Net: 0."""
        self.reverse_bytes4()                  # LE -> BE
        self.dup(); self.dup()
        self.rotr_be(2); self.swap(); self.rotr_be(13)
        self.bin_op("OP_XOR")
        self.swap(); self.rotr_be(22)
        self.bin_op("OP_XOR")
        self.reverse_bytes4()                  # BE -> LE

    def big_sigma1(self) -> None:
        """S1(e) = ROTR(6)^ROTR(11)^ROTR(25). [e(LE)] -> [S1(LE)]. Net: 0."""
        self.reverse_bytes4()
        self.dup(); self.dup()
        self.rotr_be(6); self.swap(); self.rotr_be(11)
        self.bin_op("OP_XOR")
        self.swap(); self.rotr_be(25)
        self.bin_op("OP_XOR")
        self.reverse_bytes4()

    def small_sigma0(self) -> None:
        """s0(x) = ROTR(7)^ROTR(18)^SHR(3). [x(LE)] -> [s0(LE)]. Net: 0."""
        self.reverse_bytes4()
        self.dup(); self.dup()
        self.rotr_be(7); self.swap(); self.rotr_be(18)
        self.bin_op("OP_XOR")
        self.swap(); self.shr_be(3)
        self.bin_op("OP_XOR")
        self.reverse_bytes4()

    def small_sigma1(self) -> None:
        """s1(x) = ROTR(17)^ROTR(19)^SHR(10). [x(LE)] -> [s1(LE)]. Net: 0."""
        self.reverse_bytes4()
        self.dup(); self.dup()
        self.rotr_be(17); self.swap(); self.rotr_be(19)
        self.bin_op("OP_XOR")
        self.swap(); self.shr_be(10)
        self.bin_op("OP_XOR")
        self.reverse_bytes4()
...
def _generate_compress_ops() -> "list[StackOp]":
    ...
    em = _Emitter(2)  # pretend state+block are the only items
    ...
    for t in range(64):
        d0 = em.depth
        _emit_round(em, t)
        em.assert_depth(d0, f"compress: after round {t}")
...
_compress_ops_cache: "list[StackOp] | None" = None


def _get_compress_ops() -> "list[StackOp]":
    global _compress_ops_cache
    if _compress_ops_cache is None:
        _compress_ops_cache = _generate_compress_ops()
    return _compress_ops_cache
...
def emit_sha256_compress(emit: Callable[["StackOp"], None]) -> None:
...
def emit_sha256_finalize(emit: Callable[["StackOp"], None]) -> None:
    ...
    em = _Emitter(3)  # state + remaining + msgBitLen
