 ...
 *
 * Parses `.tsop.move` files into the same TSOP AST that the TypeScript
...
// ---------------------------------------------------------------------------
// Type mapping: Move snake_case → TSOP camelCase
// ---------------------------------------------------------------------------
...
class MoveParser {
  ...
  private parsePrimary(): Expression {
    ...
    if (t.type === 'ident') {
      ...
      // Map Move builtins to TSOP builtins
...
export function parseMoveSource(source: string, fileName?: string): ParseResult {
  const file = fileName ?? 'contract.tsop.move';
