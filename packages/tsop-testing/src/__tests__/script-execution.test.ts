 ...
 *
 * These tests compile TSOP contracts with baked constructor args, build
...
function readContract(name: string): string {
  return readFileSync(resolve(CONFORMANCE, name, `${name}.tsop.ts`), 'utf8');
}
...
describe('Script execution: pure computation', () => {
  describe('Arithmetic', () => {
    ...
    it('succeeds with correct target (3+7=10, 3-7=-4, 3*7=21, 3/7=0 → 27)', () => {
      const c = ScriptExecutionContract.fromSource(src, { target: 27n }, 'arithmetic.tsop.ts');
      expect(c.execute('verify', [3n, 7n]).success).toBe(true);
    });

    it('fails with wrong target', () => {
      const c = ScriptExecutionContract.fromSource(src, { target: 0n }, 'arithmetic.tsop.ts');
  ...
  describe('BooleanLogic', () => {
    ...
    it('succeeds when both above threshold', () => {
      ...
      const c = ScriptExecutionContract.fromSource(src, { threshold: 2n }, 'boolean-logic.tsop.ts');
    ...
    it('fails when neither above threshold', () => {
      const c = ScriptExecutionContract.fromSource(src, { threshold: 10n }, 'boolean-logic.tsop.ts');
  ...
  describe('IfElse', () => {
    ...
    it('takes the true branch (value + limit > 0)', () => {
      const c = ScriptExecutionContract.fromSource(src, { limit: 10n }, 'if-else.tsop.ts');
  ...
  describe('BoundedLoop', () => {
    ...
    it('succeeds with correct expected sum', () => {
      ...
      const c = ScriptExecutionContract.fromSource(src, { expectedSum: 25n }, 'bounded-loop.tsop.ts');
      expect(c.execute('verify', [3n]).success).toBe(true);
    });

    it('fails with wrong expected sum', () => {
      const c = ScriptExecutionContract.fromSource(src, { expectedSum: 99n }, 'bounded-loop.tsop.ts');
      expect(c.execute('verify', [3n]).success).toBe(false);
    });

    it('succeeds with start=0 (sum = 0+1+2+3+4 = 10)', () => {
      const c = ScriptExecutionContract.fromSource(src, { expectedSum: 10n }, 'bounded-loop.tsop.ts');
...
describe('Script execution: signatures', () => {
  describe('P2PKH', () => {
    ...
    it('succeeds with correct key', () => {
      ...
      const c = ScriptExecutionContract.fromSource(
        src, { pubKeyHash: pubKeyHashHex }, 'basic-p2pkh.tsop.ts',
      );
  ...
  describe('MultiMethod', () => {
    ...
    it('spendWithOwner succeeds with correct key and amount > 5', () => {
      ...
      const c = ScriptExecutionContract.fromSource(
        src, { owner: ownerPub, backup: backupPub }, 'multi-method.tsop.ts',
      );
...
describe('Script execution: interpreter vs compiled agreement', () => {
  ...
  for (const tc of contracts) {
    it(`${tc.name}: interpreter and compiled script agree (success)`, () => {
      ...
      // Interpreter path
      const interp = TestContract.fromSource(src, tc.args, `${tc.name}.tsop.ts`);
      const interpResult = interp.call(tc.method, tc.params);

      // Compiled script path
      const compiled = ScriptExecutionContract.fromSource(src, tc.args, `${tc.name}.tsop.ts`);
