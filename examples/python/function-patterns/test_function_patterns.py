...
def test_withdraw_no_fee():
    c = FunctionPatterns(owner=mock_pub_key(), balance=10000)
    c.withdraw(mock_sig(), 3000, 0)  # 0 bps = no fee
...
def test_normalize_rounds_down():
    c = FunctionPatterns(owner=mock_pub_key(), balance=7777)
...
def test_normalize_clamps_up():
    c = FunctionPatterns(owner=mock_pub_key(), balance=50)
...
def test_is_positive():
    c = FunctionPatterns(owner=mock_pub_key(), balance=0)
...
def test_deposit_then_withdraw_with_fee():
    ...
    c.withdraw(mock_sig(), 5000, 200)  # 2% fee = 100, total = 5100 -> 9900
    assert c.balance == 9900


def test_scale_then_normalize():
    c = FunctionPatterns(owner=mock_pub_key(), balance=10000)
    c.scale(mock_sig(), 3, 4)              # 10000 * 3/4 = 7500
    c.normalize(mock_sig(), 0, 10000, 1000)  # clamp (no effect), round -> 7000
    assert c.balance == 7000
