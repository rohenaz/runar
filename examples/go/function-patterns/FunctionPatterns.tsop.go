...
// used inside a TSOP Go contract.
//
...
//
//   4. Built-in functions  — functions from the tsop package (e.g.
//                            tsop.Assert, tsop.Hash160, tsop.Safediv).
...
// owner, demonstrating all the function call patterns available in TSOP Go.
type FunctionPatterns struct {
	tsop.StatefulSmartContract
	Owner   tsop.PubKey `tsop:"readonly"` // immutable: contract creator
	Balance tsop.Bigint                   // stateful: current balance
}
...
func (c *FunctionPatterns) Deposit(sig tsop.Sig, amount tsop.Bigint) {
	// Built-in: signature verification
	c.requireOwner(sig)

	// Built-in: assertion with a standalone helper
	tsop.Assert(isPositive(amount))
...
func (c *FunctionPatterns) Withdraw(sig tsop.Sig, amount tsop.Bigint, feeBps tsop.Bigint) {
	c.requireOwner(sig)
...
func (c *FunctionPatterns) Scale(sig tsop.Sig, numerator tsop.Bigint, denominator tsop.Bigint) {
	c.requireOwner(sig)
...
func (c *FunctionPatterns) Normalize(sig tsop.Sig, lo tsop.Bigint, hi tsop.Bigint, step tsop.Bigint) {
	c.requireOwner(sig)
...
func (c *FunctionPatterns) requireOwner(sig tsop.Sig) {
...
func (c *FunctionPatterns) computeFee(amount tsop.Bigint, feeBps tsop.Bigint) tsop.Bigint {
	return tsop.PercentOf(amount, feeBps)
}
...
func isPositive(n tsop.Bigint) bool {
	return n > 0
}

// scaleValue computes (value * numerator) / denominator safely.
// Demonstrates a standalone helper using a built-in math function.
func scaleValue(value tsop.Bigint, numerator tsop.Bigint, denominator tsop.Bigint) tsop.Bigint {
	return tsop.MulDiv(value, numerator, denominator)
}

// clampValue constrains a value to the range [lo, hi].
// Demonstrates wrapping a built-in for readability.
func clampValue(value tsop.Bigint, lo tsop.Bigint, hi tsop.Bigint) tsop.Bigint {
	return tsop.Clamp(value, lo, hi)
}
...
func roundDown(value tsop.Bigint, step tsop.Bigint) tsop.Bigint {
	remainder := tsop.Safemod(value, step)
