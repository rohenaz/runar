...
test "SPHINCSWallet spend accepts real ECDSA and SLH-DSA authorization" {
    ...
    const slhdsa_sig = try fixtures.decodeHexAlloc(std.testing.allocator, fixtures.slhdsa_sig_hex);
    const slhdsa_pub_key = try fixtures.decodeHexAlloc(std.testing.allocator, fixtures.slhdsa_pub_key_hex);
